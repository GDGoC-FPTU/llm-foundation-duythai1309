...
def call_gemini(
    ...
    start = time.time()
    # FIX 2: client.generate_content() → client.models.generate_content()
    # FIX 3: contents format — pass prompt string directly
...
def call_anthropic(
    ...
    respond = client.messages.create(
        ...
        temperature=temperature,
        top_p=top_p,
        # FIX 4: "message" → "messages"
        messages=[{"role": "user", "content": prompt}]
    )

    latency = time.time() - start
    # FIX 5: respond.choices[0].message.content → respond.content[0].text (Anthropic API)
    response_text = respond.content[0].text

    usage = {
        "input_tokens": respond.usage.input_tokens,
        "output_tokens": respond.usage.output_tokens,
    }
...
def compare_models(prompt: str) -> dict:
    ...
    # FIX 6: calculate_cost was incorrectly defined — it returned the dict instead of just cost,
    # and the dict was nested inside the function body. Separated into a proper helper.
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        if len(history) > 6:  # Keep last 3 turns (user + assistant)
            history = history[-6:]
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    """
    # FIX 7a: "results" → "result" (match variable name); FIX 7b: added return statement
    result = []
    ...
    for result in results:
        ...
        for model_key, model_name in model_mapping.items():
            if model_key in result["results"]:
                stats = result["results"][model_key]
                response = (
                    stats["response"][:50] + "..."
